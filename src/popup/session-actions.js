...
import { CLIPBOARD_PREFIX, CLIPBOARD_MAX_CHARS, DEBUG_DUMP_PACK_PAYLOAD } from '../core/constants.js';
import { getCaptureOpts } from './preferences.js';
import { setStatus } from './ui.js';

function downloadSessionFile(host, bytes) {
...
function downloadPackDebugJson(host, packPayload) {
  const safeHost = (host || 'site').replace(/[^\w.-]+/g, '_').slice(0, 80);
...
function maybeDumpPackPayloadForDebug(host, packPayload, { stagger = false } = {}) {
...
export async function copySession() {
  ...
  try {
    ...
    console.error(e);
    setStatus(e?.message || 'Could not copy session.', 'err');
  }
}
...
export async function exportFile() {
  ...
  try {
    ...
    console.error(e);
    setStatus(e?.message || 'Export failed.', 'err');
  }
}

export async function pasteSession() {
  ...
  try {
    ...
    setStatus(map[e?.message] || e?.message || 'Paste failed.', 'err');
  }
}
...
export async function importFile(file) {
  ...
  try {
    ...
    setStatus(map[e?.message] || e?.message || 'Import failed.', 'err');
  }
}
